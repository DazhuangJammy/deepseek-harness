...
import type { ExpertPreset, ExpertConfig, PresetRoot } from '@deepseek-ai/dsh-agent-preset-registry'
...
describe('expert prompt files', () => {
  ...
  it('recognizes ordinary presets and rejects missing or malformed expert files', async () => {
    ...
    roots.push(root)
    const preset: ExpertPreset = { id: 'ordinary', trust: 'user', path: join(root, 'ordinary', 'agent.cordis.yml') }
    ...
    const invalidPath: ExpertPreset = {
  ...
  it('creates an expert without optional icon metadata', async () => {
    ...
    const preset: ExpertPreset = { id: 'plain', trust: 'user', path: join(root, 'plain', 'agent.cordis.yml') }
