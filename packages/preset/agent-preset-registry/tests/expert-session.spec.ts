...
// The catch-all `plugin` kind is gone; a producer declares its own, so the
// fixture below declares the one this spec emits.
declare module '@deepseek-ai/dsh-llm' {
  interface MessageSourceMap {
    'projection-fixture': { kind: 'projection-fixture' } & ContextFormed
  }
}
...
describe('expert refinement projection', () => {
  ...
  it('ignores producer context and empty messages while retaining prompt-free assistant evidence', () => {
    ...
    state = definition.apply(state, {
      ...
      data: createUserMessage({
        ...
        source: { kind: 'projection-fixture' },
      }),
      surfaceOp: 'append',
    })
