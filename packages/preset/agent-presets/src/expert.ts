...
import type { AgentPreset, ExpertConfig, PresetRoot } from './preset.ts'
import type {
...
function directoryOf(preset: AgentPreset): string {
 ...
 */
export async function readExpertSummary(preset: AgentPreset): Promise<ExpertSummary | undefined> {
 ...
 */
export async function readExpertDocument(preset: AgentPreset): Promise<ExpertDocument> {
 ...
 */
export async function synchronizeExpertComposition(preset: AgentPreset): Promise<boolean> {
 ...
 */
export async function findExpertVersion(preset: AgentPreset, prompt: string): Promise<number | undefined> {
  const dir = directoryOf(preset)
  const metadata = await readMetadata(preset.id, dir)
