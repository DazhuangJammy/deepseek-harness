...
The normal packaging path is one complete command. It performs release preparation before creating the host platform's installers and update metadata. Every target requires a reverse-DNS `DSH_DESKTOP_APP_ID`. macOS targets additionally require the electron-builder certificate qualifier in `DSH_DESKTOP_MACOS_SIGNING_IDENTITY`, its 10-character Apple Team ID in `DSH_DESKTOP_MACOS_TEAM_ID`, and one complete notarytool credential strategy; the unsigned macOS command below is the local exception. The App Store Connect API-key strategy uses these variables:
...
Use the complete unsigned packaging command for local installation testing on macOS:
...
The command requires `DSH_DESKTOP_APP_ID` and the normal build dependencies. It writes the DMG and ZIP to `.desktop-build/targets/mac-arm64/unsigned-artifacts/`, signs the application bundle ad-hoc while leaving the prepared runtime with the signatures its source packages already carry, omits automatic-update configuration, and creates no release completion record. Ad-hoc signing keeps the bundle's own signature valid, so a quarantined copy asks for one explicit Open confirmation instead of being reported as damaged; re-signing the bundled runtime binaries would leave executables the kernel refuses to start. One electron-builder pass emits both formats because an unsigned build has no notarization lane to qualify. macOS quarantines downloaded applications rather than locally built ones, so an artifact built and opened on this machine launches directly, while a copy transferred to another machine needs one explicit Open confirmation. Set `ELECTRON_MIRROR` to a reachable Electron mirror when the build machine cannot reach the default download host; packaging fetches the Electron binary through it. The signed packaging and upload commands retain their release requirements.
