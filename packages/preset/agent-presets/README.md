...
| `experts` | see generated catalog | Limits for expert names, welcome messages, prompt bytes, and refinement evidence |
...
Prompt refinement starts an in-process one-shot child Agent with the complete `standard` preset and the current Session's selected model route. The coordinator adds no persona, request-size, output-token, retry, or deadline override, so the standard composition owns the child's prompt, tools, skills, compaction, delegation, workflows, goals, and error recovery; only caller cancellation and parent maintenance can stop the run early. A compact Chinese first user message explicitly invokes the bundled `expert-prompt-refiner` skill; a preceding plugin-sourced `promptContext` carries the complete expert system prompt plus the configured recent window of user inputs and assistant final-text answers ending at the selected answer. Assistant reasoning blocks and embedded reasoning stream records remain only in the source Session and are never copied into refinement evidence. The child Session records both supplied inputs with the normal Agent, skill, and model events. The Chinese skill requires an explicit correction and a later improved result, preserves unrelated prompt text, and calls the existing `structured_output` tool once with the complete candidate plus Chinese summary and reasons. The Host derives exact replacement text from the original and candidate prompts, then holds the candidate without changing expert files. Acceptance commits the user-reviewed prompt, appends a version, and selects that prompt for the current Session's later turns. A durable prompt-application event restores manually saved, accepted, or branch-selected versions without rewriting the system prompt attached to earlier answers; the next model request records the existing system-prompt update node when the text changed.
