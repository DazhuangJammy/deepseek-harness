...
A request is validated against the provider's advertised capabilities, a durable descriptor is snapshotted, and the provider builds the child. Both in-process providers advertise `agentOptions` and `promptContext`: child creation merges requested route fields over the parent's latest logged request, injects supplied context without waking the child, and then submits the ordinary prompt so both messages enter its first step in that order. The spawn provider also advertises `agentPreset`: it mounts that preset in the child creation window and records it in the child Session header; omission joins the parent's live composition. A route change without an explicit effort clears the inherited route-owned effort so the selected model resolves its default. DSH SDK also advertises `agentOptions` and publishes immutable `agentRouteDefaults`, which supply its instance provider/model defaults before exact-route preflight; `start()` still owns direct callers and the output cap. ACP, Codex, and Claude Code reject unsupported options rather than silently ignoring them. On success the run is published and ownership transfers to the caller; on failure the provider rolls back every unpublished resource. The result carries the child's final output, an optional structured value, a stop reason, and an optional safe diagnostic.
