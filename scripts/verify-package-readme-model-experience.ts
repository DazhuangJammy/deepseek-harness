...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/experimental/desktop-packager': { kind: 'none', reason: 'Host-side runner for a human-initiated local packaging build; registers nothing model-facing.' },
  'packages/experimental/client-ui-desktop-packager': { kind: 'none', reason: 'Browser-side Settings tab and /desktop command over that runner; registers nothing model-facing.' },
  'packages/experimental/desktop-packager-web-profile': { kind: 'none', reason: 'The bundle is a patch-list carrier; each inserted row\'s package owns its Model Experience.' },
