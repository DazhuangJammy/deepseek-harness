...
| `experts` | 见生成目录 | 专家名字、欢迎语、提示词字节数与优化证据的限制 |
...
提示词优化会启动一个进程内一次性子 Agent，完整使用 `standard` preset 和当前 Session 选择的模型路线。协调器不会额外设置 persona、请求大小、输出 token、重试或截止时间，因此由标准组装决定子级的提示词、工具、skill、压缩、委托、工作流、目标与错误恢复；只有调用方取消和 parent 维护信号可以提前停止这次运行。一条精简的中文首条用户消息会显式调用随附的 `expert-prompt-refiner` skill；位于其前方、由插件提供的 `promptContext` 携带完整专家系统提示词，以及已配置的近期证据窗口：其中包含截至选中回答的用户输入与助手最终文本答案。助手 reasoning 块和内嵌 reasoning stream 记录只保留在来源 Session 中，绝不会被复制到优化证据。子 Session 会把已提供的两项输入连同普通 Agent、skill 与模型事件一起记录。中文 skill 要求先有明确修正、再有改好后的结果，保留无关提示词，并调用一次既有 `structured_output` 工具，提交完整候选稿以及中文 summary 和 reasons。Host 会根据原版与候选提示词确定精确替换文本，再在不修改专家文件的情况下暂存候选稿。确认会提交用户检查后的提示词、追加版本，并让当前 Session 的后续轮次使用该提示词。持久化的提示词应用事件会在恢复时重建手动保存、确认或分支选择的版本，但不会改写旧回答对应的系统提示词；如果文本发生变化，下一次模型请求会记录既有的系统提示词更新节点。
