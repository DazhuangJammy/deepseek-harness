...
describe('catalog-addressed navigation', () => {
  it('opens an embedded child event window without changing the main selection', async ({ bench }) => {
    const b = bench()
    ...
    b.svc.open(sid('main'))
    await vi.waitFor(() => {
      expect(b.mock.log.requests(FOLLOW).some((request) => {
        const address = (request as SessionFollowRequest).address
        return address.kind === 'session' && address.sessionId === sid('main')
      })).toBe(true)
    })
    ...
    expect(b.mock.log.requests(FOLLOW)).toContainEqual(expect.objectContaining({
      address: {
