...
describe('select', () => {
  ...
  it('surfaces a failed secondary action without consuming the token', async () => {
    const deps = makeDeps()
    const { popup } = await readyPopup({
      options: () => Promise.resolve([ACTIONABLE]),
      onSecondaryAction: () => { throw new Error('editor refused') },
    }, deps)

    await popup.selectSecondary(0)

    expect(popup.state.getSnapshot()).toMatchObject({ open: true, submitting: false, error: 'editor refused' })
    expect(deps.consume).not.toHaveBeenCalled()
  })
