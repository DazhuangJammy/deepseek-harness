...
One-shot children run once and settle with a single result, plus an optional structured output and a safe diagnostic on failure. A start request may override the child Agent's provider, model, reasoning effort, and output-token limit through `agentOptions`; it may request an explicit `agentPreset` instead of inheriting the parent's composition, or supply one logged `promptContext` message that the child claims before its ordinary prompt. Every requested option requires the provider's matching capability. The in-process spawn provider supports explicit presets and prompt context; unsupported providers reject the request. Continuable children keep a durable session and accept later messages in order: the caller receives a stable child id, sends adjacent-Agent messages, and can interrupt the current turn without destroying the child. The tool row's `backgroundMode` picks the shape (`one-shot` by default, or `continuable` on providers that support it).
...
A request is validated against the provider's advertised capabilities, a durable descriptor is snapshotted, and the provider builds the child. Both in-process providers advertise `agentOptions` and `promptContext`: child creation merges requested route fields over the parent's latest logged request, falls back to creation options before the first request, retains the configured token limit, injects supplied context without waking the child, and then submits the ordinary prompt so both messages enter its first step in that order. The spawn provider also advertises `agentPreset`: it mounts that preset in the child creation window and records it in the child Session header; omission joins the parent's live composition. Both providers snapshot delegated permission state before the first await: an Auto or Full access parent gives the child the same `permission/preset` identity, while the existing sandbox override and approval-policy pin continue to apply. Recording both identities prevents an older same-bundle fork value from winning. Auto reviews every supported child call independently: ordinary project-local work is low risk and allowed; medium-risk work requires explicit action, exact-target and scope authorization from the existing creation prompt or an authenticated human/direct-parent message, without conflicting human limits; high-risk work is always denied. The reviewer derives that context from `parentSession` and existing messages; delegation adds no parent call metadata, delegation records, review receipt, or Session format. A route change without an explicit effort clears the inherited route-owned effort so the selected model resolves its default. DSH SDK advertises `agentOptions` and publishes immutable `agentRouteDefaults`, which supply its instance provider/model defaults before exact-route preflight, but its separate child runtime does not inherit Auto; `start()` still owns direct callers and the output cap. ACP, Codex, and Claude Code reject unsupported options and retain their own permission systems after the parent delegation call passes review. On success the run is published and ownership transfers to the caller; on failure the provider rolls back every unpublished resource. The result carries the child's final output, an optional structured value, a stop reason, and an optional safe diagnostic.
