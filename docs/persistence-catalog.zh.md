...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`event:expert/prompt-applied.data`](#persistence-type-eventexpertprompt-applieddata) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
...
| `expertId` | 必需 | `string` |
| `prompt` | 必需 | `string` |
| `source` | 必需 | [`packages/preset/agent-presets/src/expert-session.ts#ExpertPromptApplicationSource`](#persistence-type-packagespresetagent-presetssrcexpert-sessiontsexpertpromptapplicationsource) |
| `version` | 必需 | `number` |
...
来源：[`packages/preset/agent-presets/src/expert-session.ts:11`](../packages/preset/agent-presets/src/expert-session.ts)
