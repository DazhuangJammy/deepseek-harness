...
export class PopupSelectController<TCtx = unknown> {
  ...
  /**
   * Run one filtered row's secondary action through the same single-flight
   * settlement: the option itself is never selected, and no settlement starts
   * unless the row carries an action and the spec implements it.
   * @param index - filtered-row index.
