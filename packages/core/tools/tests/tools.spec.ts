...
describe('ToolRuntime', () => {
  it('shares its scheduler key across separately loaded package copies', async () => {
    const ctx = await setup()
    const siblingKey = Symbol.for('@deepseek-ai/dsh-tools.scheduler')
    const symbolProperties = ctx.tools as unknown as Record<symbol, unknown>
    expect(TOOL_RUNTIME_SCHEDULER).toBe(siblingKey)
    expect(symbolProperties[siblingKey]).toBe(ctx.tools[TOOL_RUNTIME_SCHEDULER])
  })
