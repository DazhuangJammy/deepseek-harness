...
describe('PopupSelectView', () => {
  ...
  it('gives the primary button of an actionable row the row selection', async () => {
    const seen: SelectOption[] = []
    const { view } = await mountOpen({
      options: () => Promise.resolve([ACTIONABLE]),
      onSelect: (option) => { seen.push(option) },
    })
    // The rows become listitems, so the option's own name is the primary button.
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: 'Interview expertv2' })) })
    expect(seen).toEqual([ACTIONABLE])
    expect(view.container.childElementCount).toBe(0)
  })
