  ...
  it('sends the complete prior transcript on each `chat` request', async () => {
    ...
    const disposeStream = handle.agent.ctx.on('llm/stream', (request) => {
      const system = request.messages
        ...
        .join('')
      if (system !== CHAT_PROMPT) return textStream('chat history test')
    ...
    try {
      ...
      expect(requests[1]?.messages.filter(message => message.role !== 'system').map(message => ({
        role: message.role,
...
describe('a composition that configures its own preset roots', () => {
  ...
  it('keeps configured roots alongside the always-prepended shipped root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort()).toEqual(['chat', 'cordis', 'minimal', 'ptc', 'standard', 'team-spec'])
